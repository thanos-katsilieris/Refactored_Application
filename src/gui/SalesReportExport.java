...
public class SalesReportExport extends JDialog {
	...
	private SalesAndReceiptEditor salesAndReceiptEditor;
	...
	public SalesReportExport(final SalesAndReceiptEditor sw, Salesman salesman,double tSales,int tItems,
			float shirtS,float skirtS,float trousersS,float coatsS,double com) {
		salesAndReceiptEditor = sw;
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(438, 166, 163, 44);
		...
		}
		{
			...
			txtReportButton.setBounds(436, 32, 163, 47);
			resultWindowPanel.add(txtReportButton);
			
			JButton htmlButton = new JButton("\u0395\u03BE\u03B1\u03B3\u03C9\u03B3\u03AE \u03C3\u03B5 HTML");
			...
			htmlButton.setFont(new Font("Times New Roman", Font.PLAIN, 14));
			htmlButton.setBackground(SystemColor.controlHighlight);
			htmlButton.setBounds(437, 102, 163, 44);
			resultWindowPanel.add(htmlButton);
	...
	private void outputTXTButtonPressed(ActionEvent evt) throws Exception {
		pathChooser("txt");
	}
	
	private void outputXMLButtonPressed(ActionEvent evt) throws Exception {
		pathChooser("xml");		
	}
	
	private void outputHTMLButtonPressed(ActionEvent evt) throws Exception {
		pathChooser("html");
	}
	
	private void pathChooser(String format) throws Exception {
		JFileChooser chooser = new JFileChooser();
		String path = null;
		int returnVal = chooser.showSaveDialog(null);

		if (returnVal == JFileChooser.APPROVE_OPTION) {
			path = chooser.getSelectedFile().toString() + "."+format;
		}
		...
		if(format.equals("txt")) {
			TXTSalesReport makeTXTFile = new TXTSalesReport(selectedSalesman,path);
			makeTXTFile.extractSalesReport();
		}else if(format.equals("xml")) {
			XMLSalesReport makeXMLFile = new XMLSalesReport(selectedSalesman,path);
			makeXMLFile.extractSalesReport();
		}else {
			HTMLSalesReport makeHTMLFile = new HTMLSalesReport(selectedSalesman,path);
			makeHTMLFile.extractSalesReport();
		}
	...
	private void cancelButtonPressed(ActionEvent evt) {
		salesAndReceiptEditor.setVisible(true);
		this.dispose();	
	}
